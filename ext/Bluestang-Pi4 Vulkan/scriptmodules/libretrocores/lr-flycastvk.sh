...
function build_lr-flycastvk() {
    ...
    isPlatform "vk" && params+=("HAVE_VULKAN=1" "HAVE_OIT=1")
    make "${params[@]}" clean
    CFLAGS+=" ${add_flags[@]}" make "${params[@]}"
    md_ret_require="$md_build/flycast_libretro.so"
}

function install_lr-flycastvk() {
    md_ret_files=(
        'flycast_libretro.so'
        'LICENSE'
    )
...
function configure_lr-flycastvk() {
    mkRomDir "dreamcast"
